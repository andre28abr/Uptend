...
public enum RemediationMap {
    ...
    private static let table: [String: (String?, RemediationEffort)] = [
        ...
        "ssh-maxauthtries":     ("echo 'MaxAuthTries 4' | sudo tee -a /etc/ssh/sshd_config && sudo systemctl reload ssh", .quick),
        ...
        "ssh-idle-timeout":     ("printf 'ClientAliveInterval 300\\nClientAliveCountMax 3\\n' | sudo tee -a /etc/ssh/sshd_config && sudo systemctl reload ssh", .quick),
        ...
        "ssh-logingrace":       ("echo 'LoginGraceTime 60' | sudo tee -a /etc/ssh/sshd_config && sudo systemctl reload ssh", .quick),
        ...
        "ssh-tcp-forward":      ("echo 'AllowTcpForwarding no' | sudo tee -a /etc/ssh/sshd_config && sudo systemctl reload ssh", .quick),
